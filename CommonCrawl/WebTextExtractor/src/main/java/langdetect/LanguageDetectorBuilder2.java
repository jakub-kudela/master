 ...
 */
public class LanguageDetectorBuilder2 {

    private static final double ALPHA_DEFAULT = 0.5;

    @NotNull private final NgramExtractor ngramExtractor;
    ...
    @Nullable private Map<LdLocale, Double> langWeightingMap;
    @NotNull private final Set<LanguageProfile> languageProfiles = new HashSet<>();
    @NotNull private final Set<LdLocale> langsAdded = new HashSet<>();

    public static LanguageDetectorBuilder2 create(@NotNull NgramExtractor ngramExtractor) {
        return new LanguageDetectorBuilder2(ngramExtractor);
    }

    private LanguageDetectorBuilder2(@NotNull NgramExtractor ngramExtractor) {
        this.ngramExtractor = ngramExtractor;
    }


    public LanguageDetectorBuilder2 alpha(double alpha) {
        if (alpha<0 || alpha>1) throw new IllegalArgumentException("alpha must be between 0 and 1, but was: "+alpha);
        this.alpha = alpha;
        return this;
    }

    public LanguageDetectorBuilder2 seed(long seed) {
        return seed(Optional.of(seed));
    }
    public LanguageDetectorBuilder2 seed(@NotNull Optional<Long> seed) {
        this.seed = seed;
        return this;
    }
     ...
     */
    public LanguageDetectorBuilder2 shortTextAlgorithm(int shortTextAlgorithm) {
     ...
     */
    public LanguageDetectorBuilder2 affixFactor(double affixFactor) {
     ...
     */
    public LanguageDetectorBuilder2 prefixFactor(double prefixFactor) {
     ...
     * @see #prefixFactor(double)
     */
    public LanguageDetectorBuilder2 suffixFactor(double suffixFactor) {
     ...
     * then set one.
     */
    public LanguageDetectorBuilder2 probabilityThreshold(double probabilityThreshold) {
     ...
     */
    public LanguageDetectorBuilder2 minimalConfidence(double minimalConfidence) {
     ...
     */
    public LanguageDetectorBuilder2 languagePriorities(@Nullable Map<LdLocale, Double> langWeightingMap) {
     ...
     */
    public LanguageDetectorBuilder2 withProfile(LanguageProfile languageProfile) throws IllegalStateException {
        ...
        for (Integer gramLength : ngramExtractor.getGramLengths()) {
            if (!languageProfile.getGramLengths().contains(gramLength)) {
                throw new IllegalArgumentException("The NgramExtractor is set to handle " + gramLength + "-grams but "
                		+ "the given language profile for "+languageProfile.getLocale()+" does not support this!");
            }
        }
     ...
     * @throws IllegalStateException if a profile for the same language was added already (must be a userland bug).
     */
    public LanguageDetectorBuilder2 withProfiles(Iterable<LanguageProfile> languageProfiles) throws IllegalStateException {
    ...
    public LanguageDetector build() throws IllegalStateException {
        ...
        return new LanguageDetectorImpl2(
