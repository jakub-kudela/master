 ...
 */
public final class LanguageDetectorImpl2 implements LanguageDetector {

    private static final Logger logger = LoggerFactory.getLogger(LanguageDetectorImpl2.class);
    ...
    /**
     * Use the {@link LanguageDetectorBuilder2}.
     */
    LanguageDetectorImpl2(
        ...
        this.priorMap = (langWeightingMap==null) ? null : Util2.makeInternalPrioMap(langWeightingMap, ngramFrequencyData.getLanguageList());
    ...
    private double[] detectBlockLongText(List<String> ngrams) {
        ...
        for (int t = 0; t < N_TRIAL; ++t) {
            ...
            for (int i=0; i<ITERATION_LIMIT; i++) {
                ...
                if (i % 5 == 0) {
                    if (Util2.normalizeProb(prob) > CONV_THRESHOLD) break; 
    ...
    private double[] initProbability() {
    	/* Note: Reverted to original code. The experimental one is commented below.
    ...
    private boolean updateLangProb(@NotNull double[] prob, @NotNull String ngram, int count, double alpha) {
        ...
        if (logger.isTraceEnabled()) logger.trace(ngram + "(" + Util2.unicodeEncode(ngram) + "):" + Util2.wordProbToString(langProbMap, ngramFrequencyData.getLanguageList()));
    ...
    private List<DetectedLanguage> sortProbability(double[] prob) {
    	/* Note: Original code is commented below.
    	...
    	*/
        ...
        for (int j=0;j<prob.length;++j) {
            ...
            if (!Double.isInfinite(p) && p >= probabilityThreshold) {
                for (int i=0; i<=list.size(); ++i) {
                    if (i == list.size() || list.get(i).getProbability() < p) {
                        list.add(i, new DetectedLanguage(ngramFrequencyData.getLanguage(j), p));
                        break;
