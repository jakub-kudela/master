...
void CountWordsFromTrainFile(struct train_params *params) {
  ...
  while (1) {
    ...
    if (feof(fin)) break;
    params->train_words++;
    if ((debug_mode > 1) && (params->train_words % 1000000 == 0)) {
      printf("%lldK\n", params->train_words / 1000);
      fflush(stdout);
    }
  }
  if (debug_mode > 0) {
...
void *TrainModelThread(void *id) {
  ...
  while (1) {
    ...
    if (src_word_count - src_last_word_count > 100000) {
      src->word_count_actual += src_word_count - src_last_word_count;
      src_last_word_count = src_word_count;
      if ((debug_mode > 1)) {
        now=clock();
        if (is_bi){
          printf("Alpha: %f, bi_alpha: %f,  Progress: %.2f%%  Words/thread/sec: %.2fk\n", alpha, bi_alpha,
...
void TrainModel() {
  ...
  for(cur_iter=start_iter; cur_iter<num_train_iters; cur_iter++){
    ...
    fprintf(stderr, "# Done iter %d, alpha=%f, ", cur_iter, alpha); execute("date"); fflush(stderr);
